...
describe('List Billings Based on "value" Query Parameter', () => {
  beforeAll(async () => {
  ...
  it('should be possible to filter billings by the specified amount', async () => {
    ...
    expect(response.body[0]).toHaveProperty('value', value);
  });
  ...
  it('should fail if the value passed is not a positive integer', async () => {
    ...
    expect(response.body).toHaveProperty(
      'message.query.value',
      'query.value deve ser um número inteiro',
    );
  });
});
